...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    #check correctness of the cost
    cost = - np.sum(np.multiply(y, np.log(y_hat)))
    gradCost = y_hat - y
    gradPred = np.dot(gradCost, outputVectors)
    grad = np.outer(gradCost, predicted)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    output = [word2vecCostAndGradient(inputVectors[centerIndex], i, outputVectors, dataset) for i in contextIndices]

    cost, gradIn[centerIndex], gradOut = (sum(x) for x in zip(*output))
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    centerIndex = tokens[currentWord]
    contextIndices = [tokens[w] for w in contextWords]
    predicted = sum(inputVectors[i] for i in contextIndices)
    cost, grad, gradOut = word2vecCostAndGradient(predicted, centerIndex, outputVectors, dataset)
