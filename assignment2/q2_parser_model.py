...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        pretrained_embeddings = tf.get_variable("pretrained_embeddings", initializer=self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("forward_pass"):
            self.W = W = tf.get_variable(name='W', initializer=xavier_initializer([self.config.n_features * self.config.embed_size, self.config.hidden_size]))
            # zeros initialization for biases does not work!!!
            b1 = tf.get_variable('b1', initializer=tf.random_uniform([self.config.hidden_size]))

            h = tf.nn.relu(tf.matmul(x, W) + b1)
            h_drop = tf.nn.dropout(h, self.dropout_placeholder)


            # second hidden layer increases performance
            self.W2 = W2 = tf.get_variable(name='W2', initializer=xavier_initializer([self.config.hidden_size, self.config.hidden_size_2]))
            b2 = tf.get_variable('b2', initializer=tf.random_uniform([self.config.hidden_size_2]))
            h2 = tf.nn.relu(tf.matmul(h_drop, W2) + b2)
            h_drop_2 = tf.nn.dropout(h2, self.dropout_placeholder)



            self.U = U = tf.get_variable(name='U', initializer=xavier_initializer([self.config.hidden_size_2, self.config.n_classes]))
            b3 = tf.get_variable('b3', initializer=tf.random_uniform([self.config.n_classes]))
            pred = tf.matmul(h_drop_2, U) + b3
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        # regularization does not bring improvement
        # regularization = self.config.beta * (tf.nn.l2_loss(self.W) + tf.nn.l2_loss(self.W2))
        # loss += regularization
        ### END YOUR CODE
        return loss
